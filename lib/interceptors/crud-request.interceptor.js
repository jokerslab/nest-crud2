...
let CrudRequestInterceptor = class CrudRequestInterceptor extends crud_base_interceptor_1.CrudBaseInterceptor {
    intercept(context, next) {
        ...
        try {
            ...
            throw error instanceof crud_request_1.RequestQueryException ? new common_1.BadRequestException(error.message) : error;
        }
    }
    ...
    getParamsSearch(parser, crudOptions, params) {
        if (params) {
            ...
            return (0, util_1.isArrayFull)(parser.paramsFilter) ? parser.paramsFilter.map(parser.convertFilterToSearch) : [];
        }
        return [];
    }
