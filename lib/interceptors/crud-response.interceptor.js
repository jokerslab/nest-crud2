...
let CrudResponseInterceptor = class CrudResponseInterceptor extends crud_base_interceptor_1.CrudBaseInterceptor {
    ...
    transform(dto, data) {
        ...
        return data instanceof dto ? (0, class_transformer_1.classToPlain)(data) : (0, class_transformer_1.classToPlain)((0, class_transformer_1.classToPlainFromExist)(data, new dto()));
    }
    serialize(context, data) {
        ...
        switch (action) {
            case enums_1.CrudActions.ReadAll:
                return isArray ? data.map((item) => this.transform(serialize.get, item)) : this.transform(dto, data);
            case enums_1.CrudActions.CreateMany:
                return isArray ? data.map((item) => this.transform(dto, item)) : this.transform(dto, data);
            default:
