...
exports.CrudRoutesFactory = void 0;
...
class CrudRoutesFactory {
    ...
    mergeOptions() {
        ...
        this.options.auth = (0, util_1.isObjectFull)(authOptions) ? authOptions : {};
        if ((0, util_1.isUndefined)(this.options.auth.property)) {
            this.options.auth.property = module_1.CrudConfigService.config.auth.property;
        }
        const query = (0, util_1.isObjectFull)(this.options.query) ? this.options.query : {};
        this.options.query = { ...module_1.CrudConfigService.config.query, ...query };
        const routes = (0, util_1.isObjectFull)(this.options.routes) ? this.options.routes : {};
        this.options.routes = deepmerge(module_1.CrudConfigService.config.routes, routes, {
            arrayMerge: (a, b, c) => b,
        });
        this.options.params = (0, util_1.isObjectFull)(this.options.params)
            ? this.options.params
            : (0, util_1.isObjectFull)(module_1.CrudConfigService.config.params)
        ...
        if (!(0, util_1.isObjectFull)(this.options.dto)) {
            this.options.dto = {};
        }
        const serialize = (0, util_1.isObjectFull)(this.options.serialize) ? this.options.serialize : {};
        this.options.serialize = { ...module_1.CrudConfigService.config.serialize, ...serialize };
        this.options.serialize.get = (0, util_1.isFalse)(this.options.serialize.get)
            ? false
            : this.options.serialize.get || this.modelType;
        this.options.serialize.getMany = (0, util_1.isFalse)(this.options.serialize.getMany)
            ? false
            : this.options.serialize.getMany
                ? this.options.serialize.getMany
                : (0, util_1.isFalse)(this.options.serialize.get)
                    ? false
                    : serialize_helper_1.SerializeHelper.createGetManyDto(this.options.serialize.get, this.modelName);
        this.options.serialize.create = (0, util_1.isFalse)(this.options.serialize.create)
            ? false
            : this.options.serialize.create || this.modelType;
        this.options.serialize.update = (0, util_1.isFalse)(this.options.serialize.update)
            ? false
            : this.options.serialize.update || this.modelType;
        this.options.serialize.replace = (0, util_1.isFalse)(this.options.serialize.replace)
            ? false
            : this.options.serialize.replace || this.modelType;
        this.options.serialize.delete =
            (0, util_1.isFalse)(this.options.serialize.delete) || !this.options.routes.deleteOneBase.returnDeleted
                ? false
    ...
    canCreateRoute(name) {
        ...
        if ((0, util_1.isArrayFull)(only)) {
            return only.some((route) => route === name);
        }
        if ((0, util_1.isArrayFull)(exclude)) {
    ...
    setResponseModels() {
        ...
        const models = serialize_helper_1.SerializeHelper.createJoinedResponseDto(this.modelType, this.modelName, joinTree);
        ...
        this.swaggerModels.getMany = serialize_helper_1.SerializeHelper.createGetManyDto(this.swaggerModels.get, this.modelName);
    ...
    overrideRoutes(routesSchema) {
        (0, util_1.getOwnPropNames)(this.targetProto).forEach((name) => {
            const override = reflection_helper_1.R.getOverrideRoute(this.targetProto[name]);
            const route = routesSchema.find((r) => (0, util_1.isEqual)(r.name, override));
    ...
    overrideParsedBodyDecorator(override, name) {
        const allowed = ['createManyBase', 'createOneBase', 'updateOneBase', 'replaceOneBase'];
        const withBody = (0, util_1.isIn)(override, allowed);
        ...
        if (withBody && parsedBody) {
            ...
            if ((0, util_1.isEqual)(override, 'createManyBase')) {
                ...
                const toCopy = (0, util_1.isIn)(metatype, types) || (0, util_1.isNil)(metatype);
                if (toCopy) {
    ...
    getPrimaryParams() {
        return (0, util_1.objKeys)(this.options.params).filter((param) => this.options.params[param] && this.options.params[param].primary);
    }
    ...
    setRouteArgs(name) {
        let rest = {};
        const routes = ['createManyBase', 'createOneBase', 'updateOneBase', 'replaceOneBase'];
        if ((0, util_1.isIn)(name, routes)) {
            const action = this.routeNameAction(name);
            const hasDto = !(0, util_1.isNil)(this.options.dto[action]);
            const { UPDATE, CREATE } = enums_1.CrudValidationGroups;
            const groupEnum = (0, util_1.isIn)(name, ['updateOneBase', 'replaceOneBase']) ? UPDATE : CREATE;
    ...
    setRouteArgsTypes(name) {
        if ((0, util_1.isEqual)(name, 'createManyBase')) {
        ...
        else if ((0, util_1.isIn)(name, ['createOneBase', 'updateOneBase', 'replaceOneBase'])) {
            const action = this.routeNameAction(name);
    ...
    setInterceptors(name) {
        ...
        reflection_helper_1.R.setInterceptors([
            ...
            ...((0, util_1.isArrayFull)(interceptors) ? interceptors : []),
        ], this.targetProto[name]);
    }
    setDecorators(name) {
        const decorators = this.options.routes[name].decorators;
        reflection_helper_1.R.setDecorators((0, util_1.isArrayFull)(decorators) ? decorators : [], this.targetProto, name);
    }
    setAction(name) {
    ...
    setSwaggerPathParams(name) {
        ...
        const withoutPrimary = ['createManyBase', 'createOneBase', 'getManyBase'];
        const removePrimary = (0, util_1.isIn)(name, withoutPrimary);
        const params = (0, util_1.objKeys)(this.options.params)
