...
class SerializeHelper {
    ...
    static createSimpleDto(dto, resourceName, joinTree) {
        const objRelations = (0, typeorm_1.getMetadataArgsStorage)().relations.filter((relation) => relation.target === dto);
        const related = objRelations.map((relation) => relation.propertyName);
        class SimpleResponseDto extends (0, swagger_1.OmitType)(dto, related) {
        }
        ...
        if (relatedKeys.length > 0) {
            ...
            if (rel) {
                ...
                swagger_helper_1.Swagger.setExtraModels({ get: RelatedResponseDto });
                (0, swagger_helper_1.ApiPropertyProg)({ type: RelatedResponseDto, required: false }, SimpleResponseDto.prototype, relatedKeys[0] + 'jiggle');
            }
            else {
