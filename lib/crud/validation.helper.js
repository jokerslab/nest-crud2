...
const validator = (0, util_2.safeRequire)('class-validator', () => require('class-validator'));
const transformer = (0, util_2.safeRequire)('class-transformer', () => require('class-transformer'));
class BulkDto {
}
class Validation {
    static getValidationPipe(options, group) {
        return validator && !(0, util_1.isFalse)(options.validation)
    ...
    static createBulkDto(options) {
        if (validator && transformer && !(0, util_1.isFalse)(options.validation)) {
            const { IsArray, ArrayNotEmpty, ValidateNested } = validator;
            const { Type } = transformer;
            const hasDto = !(0, util_1.isNil)(options.dto.create);
            ...
            __decorate([
                (0, swagger_helper_1.ApiProperty)({ type: Model, isArray: true }),
