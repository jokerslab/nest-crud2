...
class CrudService {
    ...
    getPrimaryParams(options) {
        const params = (0, util_1.objKeys)(options.params).filter((n) => options.params[n] && options.params[n].primary);
        return params.map((p) => options.params[p].field);
    }
    getPrimaryParam(options) {
        const param = (0, util_1.objKeys)(options.params).find((n) => options.params[n] && options.params[n].primary);
        if (!param) {
            return undefined;
        }
